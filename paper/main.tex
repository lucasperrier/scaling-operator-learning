...
\begin{itemize}
  ...
  \item \textbf{Data seeds}: $\{11, 22, 33\}$
  \item \textbf{Training seeds}: $\{101, 202, 303\}$
\end{itemize}
...
may shift the resolution-scaling balance. The baseline resolution grid
has only 4 training values ($R \in \{32, 64, 128, 256\}$); the
widened 10-value grid (\Cref{app:wide-res}) substantially improves
law-selection power and reveals shifts (e.g., FNO resolution scaling
moves to linear) but is currently available only for Burgers.
