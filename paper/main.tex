...
recorded. We report the dominant law (most slice-level wins), the
median $R^2$ across slices, and the median characteristic parameter.
...
\begin{itemize}
  \item \textbf{Logarithmic data scaling} signals a high-dimensional
    learning problem where the curse of dimensionality dominates. In our
    setting, Burgers and Darcy---both involving complex input
    distributions with broad spectral content---fall in this regime.
...
representational bottleneck in practice. The uniform cross-resolution
...
and used as a robustness check against AICc. BIC penalises model
complexity more heavily for large $n$, though for our small sample
sizes the two criteria often agree.
...
and BIC select the same winning law. High concordance ($>$80\%)
indicates that the law-selection conclusions are not sensitive to the
...
\vspace{4pt}
\noindent{\footnotesize The low concordance on the data axis reflects
the well-known divergence between AICc and BIC for small-$n$ slices
where AICc's weaker complexity penalty favors the logarithmic law
(2~parameters) while BIC favors it less. Capacity-axis concordance
is generally high ($>$65\%), confirming the robustness of capacity
findings. Resolution concordance varies with PDE smoothness.}
\end{table}
