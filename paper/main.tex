...
\newcommand{\holdoutWinFraction}{14/33 (42\%)}
\newcommand{\holdoutMeanImprovement}{1.0\%}
\newcommand{\withinSliceMeanFrac}{0.46}
\newcommand{\withinSliceUnstableFrac}{19/27 (70\%)}
...
(i)~\emph{data scaling is not well described by a simple power law}---with
a sparse 5-point grid, logarithmic wins in 7 of 9 combinations; with a
denser 11-point grid (\Cref{app:dense-n}), bounded-decay families
(exponential, saturation) displace logarithmic, indicating an
irreducible-error floor that the sparse grid cannot resolve;
...
\begin{enumerate}
  ...
  \item We report \textbf{29{,}860+ training runs} and
    cross-resolution evaluations across three PDE
    families, three architectures, and seven capacity levels.
  \item We find that \textbf{data scaling is not a simple power law}:
    with the baseline grid, logarithmic ($E = a - b \ln N$) dominates
    7 of 9 combinations; with a denser 11-point grid, bounded-decay
    families (exponential, saturation) emerge as the true winners,
    revealing an irreducible-error floor that the sparse grid obscures
    (see \Cref{app:dense-n}).
...
All 29{,}860 training runs (baseline grid plus dense-$N$,
wide-$R$, controlled-MLP, and ablation extensions) completed
with zero divergence. We report relative $L^2$ error
...
\paragraph{Key finding: logarithmic data scaling is the norm on the
baseline grid, but denser sampling reveals bounded-decay alternatives.}
In 7 of 9 task--model combinations with $n = 5$ data sizes, the AICc
selects the logarithmic form $E = a - b \ln N$ over the power law.
However, extending to $n = 11$ sample sizes (\Cref{app:dense-n})
shifts the winner toward bounded-decay families (exponential,
saturation) that encode an irreducible error floor. The logarithmic
result should therefore be interpreted as an \textbf{artefact of sparse
sampling}: with only five $N$ values the curvature near the floor is
unresolvable.

Regardless of the specific family, the core practical conclusion is
unchanged: error decreases
much slower than a power law $E \propto N^{-\alpha}$ would predict.

\noindent\textbf{Caveat (within-slice stability).}
The inter-slice bootstrap (Boot.\% column) shows stable \emph{aggregate}
selection, but the within-slice bootstrap (\Cref{sec:within-slice-bootstrap})
reveals that individual slices carry substantial uncertainty: mean
within-slice dominant fractions on the data axis range from 0.33 to
0.59, and in several cases the bootstrap's per-slice winner differs
from the AICc aggregate winner. All data-axis claims except
Diffusion MLP ($w_1 = 0.68$, bootstrap fraction $= 0.59$) should
therefore be interpreted as indicative rather than robust.
...
slices (baseline five-point $N$ grid). Blue: logarithmic $a - b \ln N$.
Red: power law $\Einf + a N^{-\alpha}$. Purple: exponential. Titles
show the law vote distribution across all slices. On this grid,
logarithmic fits dominate for Burgers and Darcy; power laws win
decisively for Diffusion FNO/MLP. See \Cref{app:dense-n} for results
with an eleven-point grid that shifts preference toward bounded-decay
families.}
...
of \holdoutMeanImprovement{}. In most cases the two approaches
perform comparably, with differences in the fourth decimal place.
The clearest multi-law advantage appears on the Darcy task, where
the data-axis fit for MLP improves substantially (0.343 $\to$ 0.234).
These results show that multi-law selection does not overfit to
in-sample points (comparable or better extrapolation), while
providing richer descriptive information. However, the extrapolation
advantage alone does not justify the framework---its primary value
lies in the diagnostic power of law \emph{identity}.
...
7 satisfy both criteria for robust selection (mean Akaike weight
$>0.5$ \emph{and} within-slice dominant fraction $>0.5$ for the
same law); 15 are ambiguous (one criterion met); and 5 are weak
(neither met). The resolution axis yields the most robust selections
(4 of 7 robust cases), consistent with its role as a stable
diagnostic. Data-axis and capacity-axis claims should be treated
with appropriate caution until the denser $n=11$ grid
(\Cref{app:dense-n}) provides stronger discriminative power.
...
\begin{itemize}
  \item \textbf{Logarithmic or bounded-decay data scaling} is consistent with a
    high-dimensional learning problem where the curse of dimensionality
    limits convergence rates. On the baseline five-point grid,
    logarithmic fits dominate; with the denser eleven-point grid
    (\Cref{app:dense-n}), exponential and saturation families often
    supersede them—but both families share the diagnostic signature
    of severe diminishing returns. In our setting, Burgers and
...
(linear rather than logarithmic on the baseline grid), suggesting
that FNO's spectral smoothing may truncate some high-frequency
information present at higher resolutions.

\paragraph{DeepONet.}
DeepONet consistently shows the weakest performance: highest $\Einf$
on all tasks, slow data scaling even on Diffusion (where FNO/MLP
...
selection. Across 29{,}860+ training runs spanning baseline,
dense-$N$, wide-$R$, controlled-MLP, and optimizer ablation grids,
our findings challenge several widespread assumptions:

\begin{enumerate}
  \item \textbf{Power laws are not the default.} On the baseline
    five-point grid, data scaling follows logarithmic decay
    ($E = a - b \ln N$) in 7 of 9 task--model combinations;
    with the denser eleven-point grid (\Cref{app:dense-n}),
    bounded-decay families (exponential, saturation) often supersede
    logarithmic, but power laws remain confined to Diffusion FNO/MLP
    ($\alpha \approx 1.1$, $R^2 = 0.998$).
  ...
  \item \textbf{The type of scaling law is itself a diagnostic.}
    Sub-power-law data scaling (logarithmic or bounded decay) signals
    hard learning regimes; negative logarithmic resolution scaling
    signals harmful input over-discretization; flat capacity scaling
    confirms architectural sufficiency.
...
\emph{Takeaway:} The ``logarithmic dominance'' reported in
\Cref{sec:data-scaling} is an artifact of sparse sampling;
with denser grids, data-axis scaling is better described by families
with an irreducible-error floor, consistent with finite-resolution
approximation limits. Results for Darcy FNO and all Diffusion models
are pending completion of the remaining dense-N runs.
...
linear competition. Results for Darcy and Diffusion are pending
completion of remaining wide-R runs.
...
\paragraph{Results.}
We analyse Burgers at medium capacity (756 runs per variant for
Adam~$2\times$; 471/756 complete for SGD~cosine).
