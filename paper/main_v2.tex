...
Second, \emph{dataset size is the primary useful lever}, explaining
65--93\% of error variance in a log-linear regression;
...
capacity.
We further show that the functional form of the scaling relationship
is regime-dependent: no single law (power, exponential, logarithmic)
holds across tasks and architectures.
...
\begin{enumerate}[leftmargin=*,itemsep=2pt]
  ...
  \item Evidence that the \textbf{functional form of scaling is
    regime-dependent}: AICc-based model selection across six candidate
    laws reveals no single winner, and the selected law shifts with
    grid density (\Cref{sec:law-form}).
...
comparison. The optimizer ablation
(\Cref{sec:ablations}) explicitly tests whether findings change
...
To quantify how much each axis contributes to error, we fit a
...
Dataset size explains 65--93\% of error variance for all standard
models. Capacity is a distant second. Resolution contributes less
...
\subsection{Sensitivity to Grid Density}
\label{sec:grid-sensitivity}
The shift in data-axis law selection from logarithmic (5 points)
to bounded-decay (11 points) is an important methodological warning:
\emph{scaling-law identification requires dense grids}.
At $n = 4$--5 points per slice, even AICc (designed for small
samples) cannot reliably distinguish functional forms. We recommend
a minimum of 8--10 evaluation points per axis for any future
scaling study that aims to identify the functional form, not merely
the direction.
...
\paragraph{Interpretation.}
The discontinuous permeability coefficients create a fundamentally
harder approximation problem. The highly nonlinear map from
discontinuous coefficients to smooth solutions may require much
larger datasets, architectures with attention mechanisms, or
task-specific preprocessing (e.g., log-transformed coefficients).
All scaling conclusions for Darcy should be interpreted cautiously.

\subsection{DeepONet Saturation}
\label{sec:deeponet}

DeepONet exhibits two distinct failure modes:
...
We do not claim DeepONet is fundamentally limited. The fixed
training protocol may disadvantage it, and the architecture has
shown strong results in other settings
\citep{wang2021learning}. Within our controlled protocol,
however, the saturation is systematic and reproducible.
...
200. We re-run Burgers (all three models, all capacities) under two
...
\textbf{Conclusion:} Directional findings are robust to the
training configuration. The specific functional form on the data
axis is training-sensitive, consistent with the grid-density
sensitivity discussed in \Cref{sec:grid-sensitivity}.
...
\begin{table}[h]
\centering
\caption{Summary of findings and evidence strength.}
\label{tab:evidence}
\small
...
\midrule
FNO dominates all comparisons (143/143) &
Very strong &
Use FNO for 1D operator learning \\
Dataset size is the primary lever (65--93\%) &
Strong &
...
Strong &
A modest model suffices \\
No universal scaling law form &
Moderate &
Do not assume power law; validate per task \\
DeepONet saturates early &
Strong &
Consider alternatives for data-rich regimes \\
...
\begin{enumerate}[leftmargin=*,itemsep=2pt]
  \item \textbf{Architecture first.} The choice of inductive bias
    (spectral vs.\ pointwise vs.\ MLP) has a larger effect than any
    amount of data, resolution, or capacity scaling.
...
regime-dependent. This suggests that the smooth, universal scaling
regime observed in NLP may require the problem complexity and model
scale to be in an appropriate relationship---conditions not met by
simple 1D PDEs with sub-million-parameter models.

\subsection{Limitations}
\label{sec:limitations}

\begin{enumerate}[leftmargin=*,itemsep=2pt]
  \item \textbf{1D only.} All tasks are 1D PDEs.
    Resolution may be far more important in 2D/3D, where
    discretization error compounds with dimensionality.
    We make no claims about higher-dimensional settings.

  \item \textbf{Fixed training protocol.} A shared optimizer and
    schedule ensures controlled comparison but may disadvantage
    specific architectures (e.g., DeepONet may benefit from different
    learning-rate schedules). The optimizer ablation partially
    addresses this but does not exhaust the hyperparameter space.
...
The study yields three central messages.
...
dominates all other scaling considerations by a wide margin.
...
Third, the functional form of the scaling relationship is not
universal: it depends on the task, architecture, and regime, and
shifts with the density of the evaluation grid.
...
1D benchmarks: practitioners should prioritize choosing the right
architecture and collecting sufficient training data over scaling
model size or discretization resolution.
The extent to which these conclusions transfer to higher-dimensional,
multi-physics, or industrial-scale problems remains an important
open question.
